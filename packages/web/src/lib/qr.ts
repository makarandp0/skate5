...
const byteCapacity = (spec: QrSpec): number => {
  return Math.floor((spec.dataCodewords * 8 - 12) / 8);
};

const createMatrix = <T>(size: number, value: T): T[][] => {
...
const encodeData = (data: Uint8Array, dataCodewords: number): number[] => {
  const bits: number[] = [];
...
const reserveFormatAreas = (reserved: boolean[][]): void => {
...
const multiplyFiniteField = (a: number, b: number): number => {
  let result = 0;
